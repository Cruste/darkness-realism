...
public class MessageConfig implements IMessage, IMessageHandler<MessageConfig, IMessage>
{
	...
	public MessageConfig(RealismDarknessConfig config)
	{
		this.config = config;
	}
	...
	public IMessage onMessage(final MessageConfig message, MessageContext ctx)
	{
		Minecraft.getMinecraft().addScheduledTask(new Runnable()
		{
			...
			public void run()
			{				
				RealismDarknessConfig config = message.config;
	...
	{
		this.config = new RealismDarknessConfig();
